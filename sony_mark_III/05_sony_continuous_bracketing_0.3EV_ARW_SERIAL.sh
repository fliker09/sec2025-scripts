...
function run_tether() {
    ...
    if [ "$1" == "" ]
    then
        gphoto2 --capture-tethered=CAPTURECOMPLETE --filename %Y_%m_%d_%H_%M_%S_%n.ARW
        gphoto2 --capture-tethered=CAPTURECOMPLETE --filename %Y_%m_%d_%H_%M_%S_%n.ARW
    else
        gphoto2 --capture-tethered=CAPTURECOMPLETE --filename %Y_%m_%d_%H_%M_%S_%n.ARW --set-config-value=/main/capturesettings/shutterspeed=$1
        echo
