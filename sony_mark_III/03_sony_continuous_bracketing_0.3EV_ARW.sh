...
time gphoto2 --camera="$model" --port="$port" --filename %Y_%m_%d_%H_%M_%S_%n.ARW --bulb 1 --capture-image-and-download --set-config-value=/main/capturesettings/shutterspeed=1/40
echo
time check_shutter_speed 1/40
echo

time gphoto2 --camera="$model" --port="$port" --filename %Y_%m_%d_%H_%M_%S_%n.ARW --bulb 2 --capture-image-and-download --set-config-value=/main/capturesettings/shutterspeed=1/20
echo
time check_shutter_speed 1/20
echo

# We don't set shutter speed for the next sequence here because... there is no next sequence :)
time gphoto2 --camera="$model" --port="$port" --filename %Y_%m_%d_%H_%M_%S_%n.ARW --bulb 4 --capture-image-and-download
echo
